...
<template>
  <div class="flex flex-col gap-4 h-md rounded border-1 border-neutral-800 p-4 group hover:(border-rainbow)" >
    <div class="flex items-center gap-4 p-2">
      <i :class="`text-2xl ${icon} group-hover:icon-rainbow`" />
      <h2 class="text-2xl group-hover:text-rainbow">{{ title }}</h2>
    </div>
    <p class="text-neutral-500 group-hover:text-neutral-400 flex-1">{{ text }}</p>
    <ul class="grid grid-cols-3 grid-rows-2 gap-4 justify-items-center">
      ...
      >
        <i
          :class="`text-2xl group-hover:icon-rainbow text-neutral-500 ${i.icon}`"
        />
